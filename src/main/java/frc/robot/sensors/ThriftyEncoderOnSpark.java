 ...
 */
public class ThriftyEncoderOnSpark
{
	private SparkAnalogSensor analogInput;
	private boolean inverted;
	private double positionOffset;

	public ThriftyEncoderOnSpark(SparkAnalogSensor port) {
		this.analogInput = port;
		this.inverted = false;
	...
	public double getPosition() {
		return (inverted ? -1.0 : 1.0) * ((analogInput.getVoltage() / RobotController.getVoltage3V3()) * (Math.PI * 2) - Math.PI);
	}
